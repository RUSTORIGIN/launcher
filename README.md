...
- ▶️ **One-click install & play** — extracts to a per-user folder and launches, with live progress
  and a one-time offer to import your existing Steam Rust keybinds.
- 🖥️ **Live server cards** — cover-image tiles with a status dot, name, and a live player-count bar
  (Steam A2S, refreshed ~60 s); click a card to connect.
- ⬆️ **Self-update** — checks GitHub Releases on launch; when a newer, checksum-verified build exists
