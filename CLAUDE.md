...
| `Social` | Repeatable: `Social=platform\|url` -> a clickable brand icon in the row under the hero PLAY/INSTALL buttons (opens the URL in the browser). Built-in brand marks (inline vector paths in `SocialGlyph`): `discord`, `youtube`, `tiktok`; an unknown platform key is skipped. A source that defines `Social=` lines replaces the list from the previous source. |
