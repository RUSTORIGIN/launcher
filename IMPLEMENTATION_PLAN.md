...
plus the `Avatar`/`Offline` helpers) have been **removed** (§4 T-cleanup, done).
...
| Hero action 2: **INSTALL / RESUME** + progress/status | `LinkButton()` → `StartInstall()`, `progTrack`/`statusText` | ✅ rendered (see §4 T1) |
...
Verify: window renders at 1440×860; hero (logo/wordmark/caption/tagline), PLAY, INSTALL, the
