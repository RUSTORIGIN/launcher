...
REM resulting RustOrigin.exe needs assets beside it to run. For a shippable single-file
...
"%FW%\csc.exe" /nologo /target:winexe /optimize+ /out:RustOrigin.exe %REFS% src\WpfLauncher.cs src\UpdateParsing.cs src\A2S.cs src\DiscordRpc.cs
if errorlevel 1 (
  echo.
  echo BUILD FAILED.
  popd
...
echo.
echo Built RustOrigin.exe (dev build - assets not embedded).
